     ...
     m_graph(Graph)
{
    ...
    for (int i = 0; i < m_controller->listOfTeams.count();i++)
    {
        ...
        ui->stadiumcombobox->addItem(teamname);
    }
...
void MainWindow::on_Loginbtn_clicked()
{
   ...
   if(m_controller->pass.foundpass(username) && m_controller->pass.findkey(username) == 1)
   {
   ...
   }else if (m_controller->pass.foundpass(username) && m_controller->pass.findkey(username) == 0)
   {
       QMessageBox::information(this, "Login", "Username and password is correct");
       this->ui->usernamelineedit->setText("");
       this->ui->passwordlineedit->setText("");
...
void MainWindow::on_stadiumcombobox_activated(const QString &arg1)
{
    ...
    QSqlQuery qry;
    qry.prepare("Select * from MLBINFO where TeamName = '"+arg1+"'");
    ...
    if (qry.exec())
    {
        while(qry.next())
        {
            ...
            ui->stadiumnameedit->setText(stadium);
            ui->prevstadiumlbl->setText(stadium);
...
void MainWindow::on_editstadium_clicked()
{
    ...
    m_controller->updateMlb(ui->teamlbl->text(),ui->stadiumnameedit->text(),ui->capacityspinbox->text(),
                            ...
                            ui->rooftypespinbox->currentText(),ui->prevstadiumlbl->text());
    ...
    ui->prevstadiumlbl->clear();
    }
}

void MainWindow::on_DFSbtn_clicked()
{
    outputNodes = m_graph->DFS(m_controller->getEnum(ui->tripBox->currentText()));
...
{    outputNodes = m_graph->bfsTraversal(m_controller->getEnum(ui->tripBox->currentText()));
...
void MainWindow::on_marlins_button_clicked()
{
    ...
    {
        QString stadium = m_controller->getStadiumName(static_cast<stadiums>(i));
        if(stadium != m_controller->getStadiumName(MiamiMarlins))
        {
...
void MainWindow::on_add_button_clicked()
{
    ...
    else if(trip == Start)
    {
        ...
        for(int i = 0; i < m_controller->getStadiumCount(); i++)
        {
            QString st = m_controller->getStadiumName(static_cast<stadiums>(i));
            if(st != stadium)
            {
...
void MainWindow::recursiveStadiumSort(QString current, QVector<QString> &selected)
{
    ...
    {
        QVector<int> dijkstra = m_graph->dijkstra(m_controller->getEnum(current));
        int next = nextStadium(dijkstra, selected);
        recursiveStadiumSort(m_controller->getStadiumName(static_cast<stadiums>(next)), selected);
    }
}
int MainWindow::nextStadium(QVector<int> dist, QVector<QString> &selected)
{
    ...
    for(int v = 0; v < dist.size(); v++)
    {
        ...
        {
            if(selected[i] == m_controller->getStadiumName(static_cast<stadiums>(v)))
            {
                visitable = true;
            }
        }
